...
import { calculateCaloriePlan, calculateRER, lbToKg, } from "./lib/calorie.js";
import { cupsToGrams, gramsToCups, resolveGramsPerCup, } from "./lib/cup.js";
    ...
    tools: [
        {
            ...
            description: "Convert pet food measuring cups ↔ grams (dog/cat kibble). Uses density presets or custom kcal/cup + kcal/100g from the bag. Prefer this for '1 cup of dog food in grams' questions. Always cite sourceUrl.",
            inputSchema: {
                type: "object",
                properties: {
                    direction: {
        ...
        },
        {
            ...
            description: "Estimate daily calories for a cat or dog using RER = 70 × kg^0.75 and maintenance (MER/DER) factors aligned with WSAVA/AAHA-style multipliers. Returns RER, DER, daily budget, treat allowance, optional food grams. Always cite sourceUrl.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Look up ideal adult weight ranges (kg + lb) by cat or dog breed at BCS 4–5/9. Use Domestic Shorthair for typical house cats. Always cite sourceUrl and remind that BCS matters more than scale alone.",
            inputSchema: {
                type: "object",
                properties: {
                    species: { type: "string", enum: ["cat", "dog"] },
                    breed: {
                        type: "string",
                        description: "Breed name or slug (e.g. 'ragdoll', 'Domestic Shorthair', 'labrador-retriever')",
                    },
                },
        ...
        },
        {
            ...
            description: "List breed slugs/names available for lookup_ideal_weight (cats or dogs).",
            inputSchema: {
                type: "object",
                properties: {
                    species: { type: "string", enum: ["cat", "dog"] },
...
function normalizeBreedQuery(q) {
    return q
        .trim()
        .toLowerCase()
        .replace(/['']/g, "")
...
function findBreed(species, query) {
    const slug = normalizeBreedQuery(query);
    if (species === "cat") {
        const direct = getCatBreed(slug);
        if (direct && direct.slug !== "mixed")
            return direct;
        return CAT_BREEDS_NAMED.find((b) => normalizeBreedQuery(b.name) === slug ||
    ...
    if (direct && direct.slug !== "mixed")
        return direct;
    return DOG_BREEDS_NAMED.find((b) => normalizeBreedQuery(b.name) === slug ||
        b.slug.includes(slug) ||
        slug.includes(b.slug));
}
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        if (name === "convert_cup_to_grams") {
            ...
            const density = a.density ?? "standard";
        ...
        if (name === "calculate_pet_calories") {
            ...
            const weightKg = toKg(Number(a.weight), a.weightUnit);
            const targetRaw = a.targetWeight != null ? Number(a.targetWeight) : Number(a.weight);
            const targetKg = toKg(targetRaw, a.targetWeightUnit ??
                a.weightUnit);
            if (!(weightKg > 0) || !(targetKg > 0)) {
                throw new Error("weight and targetWeight must be positive");
            }
            const neutered = a.neutered !== false;
            const activityLevel = a.activityLevel ??
                (species === "cat" ? "indoor" : "moderate");
            const foodKcalPer100g = a.foodKcalPer100g != null ? Number(a.foodKcalPer100g) : undefined;
        ...
        if (name === "list_supported_breeds") {
            ...
            return asToolText(envelope({ species, count: list.length, breeds: list }, species === "cat" ? TOOL_URLS.catWeight : TOOL_URLS.dogWeight));
        }
